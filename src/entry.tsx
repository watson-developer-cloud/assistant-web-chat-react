  ...
  return function withWebChatWithConfig<T>(
    ...
    function WithWebChat(props: WithWebChatProps<OriginalProps<T>>) {
      ...
      const webChatLoadedPromise = React.useRef(
        new Deferred<Promise<(passedWebChatConfig: WebChatConfig) => Promise<WebChatInstance>>>(),
      );

      /**
       * This function is passed as a prop. We are not able to immediately work to create an instance of web chat until
       * the external script loads so we await createWebChatInstanceReady being resolved. We also need to be able to keep
       * track of if the component is still mounted in all our promises, so we use createWebChatInstanceInternal that is
       * able to keep track of mounted state.
       */
...
export { withWebChat, WebChatConfig, WebChatInstance, AddedWithWebChatProps, WithWebChatConfig };
